	...
	public JsonArray jsonreturn(int orderid) {
		System.out.println("json entered");
			...
			try {
				...
				PreparedStatement pst = con.prepareStatement("select order_id from orderprovision where stage='PROVISIONED' and due_date<=sysdate and order_id=?");
				...
				while (rs.next()) {
					...
					//java.util.Date utilDate = formatter.parse(dd.toString());
				...
				//	pst=con.prepareStatement("select )
					...
					//System.out.println(cid+oid+stat+dd+stype+sid+quantity+pname+pid);
					 JsonCreatorNew json=new JsonCreatorNew();
						javax.json.JsonObject jsonObj=json.json_creator(cid,oid,stat,dd1,stype,sid,quantity,pname,pid);
						builder.add(jsonObj);
					}
